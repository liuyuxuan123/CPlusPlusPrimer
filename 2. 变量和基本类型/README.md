...
### 复合类型
&emsp;`复合类型(Compound type)`是指基于其他类型定义的类型。 `C++语言`有几种复合类型。这里先介绍两种： `引用`和`指针`。
#### 引用 Reference
&emsp;`引用`为对象起了另一个名字，引用类型引用(`refers to`)另外一种类型。 一般来说在初始化变量时，初始值会拷贝到新建的对象中。然而定义引用时，程序把引用和它的初始值`绑定(bind)`在一起，而不是将初始值拷贝给引用。一旦初始化完成，引用将和它的初始值对象一直绑定在一起。**`因为无法令引用重新绑定到另一个对象，因此引用必须要初始化。`**
`ADVICE:` 引用即别名。引用并非对象，想反的，它知识为一个已经存在的对象所起的另外一个名字。定义了一个引用对象之后，对其进行的所有操作都是在与之绑定的对象上进行的。

&emsp;允许在一条语句中定义多个引用，其中每个引用表示符都是以`符号&`开头：
```
...
```
#### 指针 Pointer
&emsp;`指针（Pointer）`是`"指向(Point to)"`另外一种类型的复合类型。与引用类似，指针也实现了对其他对象的间接访问。然而指针与引用相比又有很多不同点。其一，`指针本身就是一个对象，允许对指针赋值和拷贝`，而且在指针的生命周期内，它可以先后指向几个不同的对象。其二，`指针无需在定义时初始化`，和其它内置类型一样，在块作用域内定义的指针如果没有初始化，也将拥有一个不确定的值。

```
    int *ip1, *ip2;                 //ip1和ip2都是指向int型对象的指针
    double dp, *dp2;                //dp2是指向double型对象的指针，dp是double型对象
    int ival = 42;
    int* p = &ival;                 // 指针存放某个对象的地址，想要获得地址需要使用取地址符
```

`WARNING:` 指针的值一定属于下列4种状态之一：
1. 指向一个对象
2. 指向紧邻对象所占空间的下一个位置
3. 空指针，意为着指针没有指向任何对象
4. 无效指针，也就是上述情况之外的其他值

&emsp;`空指针(Null Pointer)`不指向任何对象，在试图使用一个指针之前代码可以首先检查该指针是否为空。一下列出几个生成空指针的方法:
```
    int *p1 = nullptr;
    int *p2 = 0;
    int *p3 = NULL;                 //需要#include<cstdlib>
```
&emsp;得到空指针最直接的方法就是使用字面值`nullptr`来初始化指针，这也是是`C++11`新标准刚刚引入的一种方法。过去的程序还会用一个名为`NULL`的`预处理变量(Preprocessor Variable)`来给指针赋值，这个变量在头文件`cstdlib`种定义，它的值就是`0`;

#### 理解复合类型的声明
```
    int i = 1024, *p = &i, &r = i;  // i 是一个int型的数，p 是一个int型指针，r 是一个int型引用
```
&emsp;经常有一种观点会误以为，在定义语句总，类型修饰符`(*或&)`作用与本次定义的全部变量。
```
    int* p;                         // 合法但是容易产生误导
    int* p1, p2;                    // p1 是指向int的指针，p2是int型对象
```
&emsp;涉及`指针`或`引用`的声明一般有两种写法。第一种是把修饰符和变量表示符写在一起：
```
    int *p1, *p2;                   //p1和p2都是指向int的指针
```
&emsp;这种形式着重强调具有的复合类型。第二种把修饰符和类型名写在一起，并且每条语句只定义一个变量：
```
    int* p1;                        //p1是指向int的指针
    int* p2;                        //p2是指向int的指针
```
